  ...
  public intercept(context: ExecutionContext, next: CallHandler): Observable<Response> {
    ...
    return next.handle().pipe(
      map(data => {
        ...
        this.logger.info(
          `${this.getTimeDelta(startTime)} ${request.ip} ${responseStatus} ${request.method} ${this.getUrl(request)}`
        );
        return data;
      }),
      catchError(err => {
        ...
        this.logger.error(
          `${this.getTimeDelta(startTime)} ${request.ip} ${err.status} ${request.method} ${this.getUrl(request)}`
        );
